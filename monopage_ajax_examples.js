...
//======================================================================================================================
// Initialisation : Définition des zones de tri et ajout de l'évènement sur la bouton add
//======================================================================================================================

var foosort=null;
var barsort=null;

document.getElementById('addTodo').setAttribute('onclick','addTodo()');
...
//======================================================================================================================
function modifierTodo(e){
// console.log('call to modifierTodo',e);
 var resultat=window.prompt('Modification du todo',e.target.innerHTML.htmlEntitiesBack());
 if(resultat!==null){
  resultat=resultat.trim().htmlEntities();
  if(resultat!=''){
   
   var eltsli=e.target.parentNode.getElementsByTagName('button');
   for(var j=0;j<eltsli.length;j++){
    if(eltsli[j].className=='elt1'){
     e.target.innerHTML=resultat;
    }
   }
  }
 }
...
// console.log('call to deleteTodo',e);
 var nodeToDelete=e.target.parentNode;
...
function addsmiley(e){
 ...
 for(var j=0;j<eltsli.length;j++){
  if(eltsli[j].className=='elt1'){
   var resultat=window.prompt('Ajout d\'un smiley au todo','💩 🤪 😀 😂 🤣 😉 😎 🤔 😐 🙄 😢😭 😍 🤔 🙃 🤓 🙋 🙏 🤑🤡 ✋👍👎✊🖐👏 ✈🔥🕾');
   if(resultat!==null){
    resultat=resultat.trim().htmlEntities();
    if(resultat!=''){
     var ancienContenu=eltsli[j].innerHTML;
     eltsli[j].innerHTML=resultat+eltsli[j].innerHTML;
     businessLogic();
     return;
    }
   }
...
//======================================================================================================================
function onChose(e){
// console.log('dans onChose, e=',e,',e.originalEvent.target=',e.originalEvent.target);
 target=e.originalEvent.target;
}
//======================================================================================================================
function defineSort(){
 try{foosort.destroy();}catch(e){}
 try{barsort.destroy();}catch(e){}
 var listeDesBlocs=['foo','bar'];
 var foo = document.getElementById("foo");
 foosort=new Sortable(foo,{group:{'name':'foo',put:listeDesBlocs,pull:true},animation:100,onEnd:function(evt){businessLogic(evt);},onChoose:function(evt){onChose(evt);}});
 var bar = document.getElementById("bar");
 barsort=new Sortable(bar,{group:{'name':'bar',put:listeDesBlocs,pull:true},animation:100,onEnd:function(evt){businessLogic(evt);},onChoose:function(evt){onChose(evt);}});
}
//======================================================================================================================

function businessLogic(evt){
// console.log('call to businessLogic',evt);
 // dès que quelque chose change, on fait les enregistrements, etc...
 // ...... ici ......
 // puis on reprend l'aspect technique
 
 
 if(evt && evt.hasOwnProperty('from')  && evt.hasOwnProperty('oldIndex') && evt.from==evt.to && evt.oldIndex==evt.newIndex ){
//  console.log('ici on lance le prompt en mobile');
//  console.log('Pour une raison qui m\'échappe complètement, je suis obligé de contourner un bug sur un "oouch device" (sic)');
  var elts=target.getElementsByTagName('button');
  var e={};
  e.target=target;
//  console.log(e.target.className);
  if(e.target.className=='elt1'){
   modifierTodo(e);
  }else if(e.target.className=='delTodo'){
   deleteTodo(e);
  }else if(e.target.className=='smiley'){
   addsmiley(e);
  }
  return;
 }
 // on met des href dans les éléments <a> des <li> car on veut à la fois faire du drag&drop pour trier et déplacer ainsi que du click
 var lists=['foo','bar'];
 for(var list in lists){
  var elts=document.getElementById(lists[list]).getElementsByTagName('button');
  for(var i=0;i<elts.length;i++){
   if(elts[i].className=='elt1'){
    try{
     elts[i].removeEventListener('click',modifierTodo);
    }catch(e){}
    elts[i].addEventListener('click',modifierTodo);
   }
   if(elts[i].className=='delTodo'){
    try{
     elts[i].removeEventListener('click',deleteTodo);
    }catch(e){}
    elts[i].addEventListener('click',deleteTodo);
   }
  
   if(elts[i].className=='smiley'){
    try{
     elts[i].removeEventListener('click',addsmiley);
    }catch(e){}
    elts[i].addEventListener('click',addsmiley);
   }
  }
 }
 setTimeout(defineSort,50);
}
//======================================================================================================================
//======================================================================================================================
function addTodo(){
// console.log(document.getElementById('inp5').value);
 var val=document.getElementById('inp5').value.trim().htmlEntities();
 if(val!=''){
  ...
  if(childs.length==0){
   ...
   var theFirstChild=par.firstChild;
   par.insertBefore(child, theFirstChild);
  }
